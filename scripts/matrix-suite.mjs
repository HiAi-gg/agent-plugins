...
const toolCalls = [
  ...
  [
    "query",
    { sql: "SELECT g FROM generate_series(1,1000) AS x(g)", row_limit: 5 },
    "query_g_alias",
  ],
  ["query", { sql: "SELECT 1 AS copy" }, "query_copy_alias"],
...
function probe(tool, args) {
  return new Promise((resolve) => {
    ...
    const call = (m, p) => {
      ...
      child.stdin.write(
        JSON.stringify({ jsonrpc: "2.0", id, method: m, params: p }) + "\n",
      );
    };
    id++;
    pending.set(id, () => {
...
let pass = 0,
  fail = 0;
for (const [tool, args, label] of toolCalls) {
  ...
  const gracefulUnavailable =
    tool === "slow_queries" && txt.includes("available");
  const tag = label || tool;
  if (!isErr || gracefulUnavailable) {
    console.log(`PASS ${tag}`);
    pass++;
  } else {
    console.log(`FAIL ${tag}: ${txt.slice(0, 120)}`);
    fail++;
  }
}
